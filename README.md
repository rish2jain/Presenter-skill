...
**QA pipeline.** `qa_check.py` catches out-of-bounds shapes, sub-11pt text, leftover placeholders, opaque chart backgrounds (the white-box-on-dark defect), and WCAG contrast failures — layer-aware, so text on cards is checked against the card. `render_slides.py` produces LibreOffice thumbnails for visual review (grid cells are numbered for `--numbers` cross-referencing). `pptx_lint.py` adds deck-wide cross-slide checks: anti-jiggle, page-number sequence, font inventory, and palette color whitelist.
